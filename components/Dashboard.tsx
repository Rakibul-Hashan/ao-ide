...
export const Dashboard: React.FC<DashboardProps> = ({
  ...
  const [newProjectLang, setNewProjectLang] = useState<Language>(Language.JAVASCRIPT);
  ...
  const getLangColor = (lang: Language) => {
    switch (lang) {
      case Language.JAVASCRIPT: return 'text-yellow-500'; 
      case Language.PYTHON: return 'text-blue-500';
      case Language.HTML: return 'text-orange-500';
      case Language.JAVA: return 'text-red-500';
      case Language.CPP: return 'text-purple-500';
      case Language.C: return 'text-blue-600';
      case Language.MARKDOWN: return 'text-pink-500';
      case Language.DOCKERFILE: return 'text-blue-700';
        ...
        <div className="flex items-center justify-between w-full md:w-auto space-x-0 md:space-x-3">
          ...
          <button onClick={onProfileClick} className="flex items-center space-x-3 text-right group cursor-pointer hover:bg-gray-900 p-2 rounded-lg transition-colors border border-transparent hover:border-gray-800">
             ...
             <div className="w-8 h-8 rounded-full bg-gray-800 overflow-hidden ring-2 ring-transparent group-hover:ring-accent-500 transition-all">
                {userAvatar ? <img src={userAvatar} alt="Profile" className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center bg-gray-800 text-xs font-bold text-gray-300">{userName.charAt(0)}</div>}
             </div>
          </button>
          <button onClick={onLogout} className="flex items-center space-x-2 text-gray-400 hover:text-red-400 transition-colors bg-gray-900 p-2 rounded-lg border border-gray-800 hover:border-red-400/30">
            <LogOut size={18} />
          ...
          <div className="mb-8 bg-gray-900 border border-gray-800 p-4 md:p-6 rounded-xl animate-fade-in-down shadow-2xl">
            <form onSubmit={handleCreate} className="flex flex-col md:flex-row gap-4 items-end">
              <div className="flex-1 w-full">
                <label className="block text-xs uppercase font-bold text-gray-500 mb-2">Project Name</label>
                <input
                  type="text"
              ...
              <div className="w-full md:w-48">
                <label className="block text-xs uppercase font-bold text-gray-500 mb-2">Stack</label>
                <select
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
            {filteredProjects.map((project) => (
              ...
              >
                ...
                <p className="text-xs text-gray-500 font-mono mb-6 truncate">{project.id}</p>
                
                <div className="mt-auto flex justify-between items-center text-sm border-t border-gray-800 pt-4">
                  <div className="flex items-center space-x-2 text-gray-400">
                    <FileCode size={14} />
                    <span className="text-xs font-medium">{project.files?.length || 1} Files</span>
                  </div>
                  <div className="flex items-center space-x-1 text-gray-500">
                    <Clock size={14} />
                    <span className="text-xs">{new Date(project.lastModified).toLocaleDateString()}</span>
                  </div>
                </div>
              </div>
            ))}
