...
export const Editor: React.FC<EditorProps> = ({ project, userSettings, installedExtensions, onSave, onClose, onExtensionChange, onUpdateSettings }) => {
  ...
  const handleSelect = () => {
    if (textareaRef.current) {
        ...
        if (lastWordMatch) {
            ...
            const allSuggestions = Array.from(new Set([...langKeywords, ...localVars]));
            const filtered = allSuggestions.filter(s => s.startsWith(word) && s !== word);
            
            if (filtered.length > 0) {
                setSuggestions(filtered);
                setShowSuggestions(true);
                const top = (currentLineIndex + 1) * 21; 
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    if ((e.ctrlKey || e.metaKey) && e.key === 'p') { e.preventDefault(); setIsQuickOpenOpen(true); } // Quick Open Files
    if ((e.ctrlKey || e.metaKey) && e.key === 'b') { e.preventDefault(); setSidebarWidth(prev => prev === 0 ? 250 : 0); } // Toggle Sidebar
    if ((e.ctrlKey || e.metaKey) && e.key === '`') { e.preventDefault(); setIsPanelVisible(prev => !prev); } // Toggle Panel
